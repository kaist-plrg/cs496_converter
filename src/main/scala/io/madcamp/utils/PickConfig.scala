  ...
  def apply(a: List[Applicant], size: Int): Int = {
    ...
    val res = (lines match {
      case List(
        ...
        (parse(year) ++ (parse(young) ++ parse(midOld) ++ parse(old)).map(
          ratio2Num(_)
        ) match {
          case List(yearl, yearh, yl, yh, ml, mh, ol, oh) => {
            val d = a.groupBy(ap =>
              (ap.birth / 10000) match {
                case x if x > yearh => 'y'
                case x if x < yearl => 'o'
                case _              => 'm'
              }
          ...
          ((parse(lowCode) ++ parse(midCode) ++ parse(highCode) ++ parse(
            superCode
          )).map(ratio2Num(_)) match {
            case List(ll, lh, ml, mh, hl, hh, sl, sh) => {
              val d = a.groupBy(_.coding)
              val (l, m, h, s) =
                (aux0(d, "하"), aux0(d, "중"), aux0(d, "상"), aux0(d, "최상"))
              (l, ll, lh) :: (m, ml, mh) :: (h, hl, hh) :: (s, sl, sh) :: Nil
            }
          }) ++
          ((parse(lowCoop) ++ parse(midCoop) ++ parse(highCoop)).map(
            ratio2Num(_)
          ) match {
    ...
    val max = (a
      .filterNot(_.isKaist)
      .groupBy(_.university)
      .map { case (_, as) => as.length }
      .toSet + 0).max
